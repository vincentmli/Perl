...
my %SETTINGS   = (
      #"0.0:nnn" => { filter => " 'port 21 or port 20' and 'host 10.2.72.139 or host 10.1.72.111' " },
      "0.0" => { filter => "" },
);
...
my $DESIRED_CAPTURE_SIZE      = 150;   # megabytes per capture file before rotating
...
my $LOG_MESSAGE               = "ICMP error limit reached";
...
foreach $interface (keys( %SETTINGS )) {
   ...
   start_tcpdump(
      $interface,
      "$CAPTURE_LOCATION/${interface}.dump.$SETTINGS{$interface}{rotate_number}.pcap",
      $filter
   );
...
if ($pid == 0) {
   ...
   foreach $interface (keys( %SETTINGS )) {
      if (capture_space("$CAPTURE_LOCATION/${interface}.dump.$SETTINGS{$interface}{rotate_number}.pcap") >= $DESIRED_CAPTURE_SIZE) {
